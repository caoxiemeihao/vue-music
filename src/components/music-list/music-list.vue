...
export default {
  ...
  watch: {
    scrollY(newY) {
      ...
      this.$refs.layer.style[transform] = tf
      ...
      this.$refs.filter.style[backdrop] = `blur(${blur}px)` // ios 才有 = =
      // this.$refs.bgImage.style.filter = `blur(${blur}px)` // 这个有点儿卡 19-12-01
      const style = this.$refs.bgImage.style // 引用类型
